...
1. After one more heartbeat run, update `docs/heartbeat_synthesis.md` with trend notes and reconcile it with `state/open_loops.md`.
2. Add a weekly synthesis automation only if hourly/half-hourly logs begin repeating or growing too long.
3. Add a useful-hour score behavior eval only if trend synthesis shows the manual score ledger is not enough.
